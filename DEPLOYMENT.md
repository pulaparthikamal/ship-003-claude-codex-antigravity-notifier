...
- **A stale nested `"hooks": { "Stop": [...] }` wrapper reappears inside `~/.codex/hooks.json`** — that shape is the legacy SingularityInc "Claude Notifier" schema (see AGENT_INTEGRATIONS.md's Codex section: the correct schema has event names at the document root, no wrapper). If it comes back after being cleaned up, it's almost always Settings Sync restoring old state (see above) rather than our extension writing it — check what's actually installed in `~/.vscode/extensions/extensions.json` / `~/.antigravity-ide/extensions/` before assuming our `codex.js` adapter regressed.
