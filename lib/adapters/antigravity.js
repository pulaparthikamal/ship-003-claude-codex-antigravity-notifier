...
// `toolCall` — treated as the turn finishing, unless its own `error` field
// (confirmed against antigravity.google/docs/hooks: "error (string,
// optional): the detailed... error message" on Stop) says the turn actually
// ended in failure. This is a distinct field on the *same* event, not a
// separate event name, so there's no risk of double-notifying — it only
// changes which kind that one Stop dispatch maps to.
function hasStopError(raw) {
  return typeof raw.error === 'string' && raw.error.trim().length > 0;
}

function normalize(raw) {
  if (raw.toolCall) return null;
  return toEvent(raw, hasStopError(raw) ? 'error' : 'task_complete');
}
...
function install(cliBin, cwd) {
  ...
  // which only ever touch $HOME) — without this, .agents/hooks.json shows up
  ...
  if (cwd) ensureIgnored(cwd, '.agents/hooks.json');
