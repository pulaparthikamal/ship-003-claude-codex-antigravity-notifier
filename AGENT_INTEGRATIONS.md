...
- **Events we hook**:
  ...
  | `StopFailure` | — | `error` | confirmed (`code.claude.com/docs/en/hooks`): fires **instead of** `Stop` when the turn ends via an API error (matcher values include `rate_limit`, `overloaded`, `authentication_failed`, `oauth_org_not_allowed`, `billing_error`, `invalid_request`, `model_not_found`, `server_error`, `max_output_tokens`, `unknown`) — a distinct event name, not a flag on `Stop`, so no double-notify risk |
  | `PostToolUseFailure` | — | `error` | confirmed: fires **instead of** `PostToolUse` when a tool call itself fails — dedicated failure-only event, so mapping it to `error` can't become per-tool-call noise the way hooking plain `PostToolUse` would |
- **Payload fields used**: `session_id`, `cwd`, `transcript_path`, `hook_event_name`, `notification_type`, `tool_name`, `tool_input`.
- **`error` kind was previously unreachable for Claude Code** — `StopFailure`/`PostToolUseFailure` exist in the docs but weren't registered as hooks at all until this pass, despite `error` already being a fully wired canonical kind (default config, sound picker, MCP server) with nothing upstream that could ever produce it.
...
- **`error` kind not implemented for Codex — deliberately**: unlike Claude Code (`StopFailure`/`PostToolUseFailure`) and Cursor (`status: "error"`, `postToolUseFailure`), `learn.chatgpt.com/docs/hooks` documents no dedicated failure event and no confirmed error/success field on any existing payload (the closest is a passing mention that `PostToolUse` "for Bash, also runs after commands that exit with a non-zero status" — not a named field we could safely key off). Don't guess a field name here: this codebase has twice shipped a hook keyed off a field/event name that didn't actually exist in the real payload (see the Codex hooks-schema and Antigravity idle-heuristic entries in git history), silently breaking notifications for everyone on that adapter. Revisit only with a captured real payload or explicit doc confirmation.
...
- **Mapping / gap-fill**:
  ...
  |---|---|---|
  | `Stop` | `task_complete` (or `error` — see below) | |
...
- **`error` via `Stop`'s own `error` field**: confirmed (`antigravity.google/docs/hooks`) the `Stop` payload carries an optional `error` string ("the detailed... error message"). A non-empty value there now maps the same `Stop` dispatch to `error` instead of `task_complete` — a field on the one event that already fires, not a second event, so this can't double-notify. `PostToolUse` also carries its own `error` field per the same docs, but we deliberately don't key off it: unlike `Stop`, `PostToolUse` fires on **every** tool call regardless of outcome, so treating it as an error source would reintroduce the exact per-call noise the `PreToolUse`/`PostToolUse` row above was fixed to avoid.
...
- **Schema** (its own shape, closer to Claude Code's `hooks` wrapper than Antigravity's, but with its own extra fields):
  ...
  ```
- **Events we hook**: `stop` (→ `task_complete`, or `error` — see below), `subagentStop` (→ `subagent_complete`, or `error`), `postToolUseFailure` (→ `error`), `beforeShellExecution`/`beforeMCPExecution` (→ `permission`, best-effort — see below), `afterAgentResponse` (idle-heuristic trigger only, never dispatches directly).
- **Payload fields used**: `conversation_id` (session id — Cursor has no `session_id` field), `workspace_roots` (array, not a single `cwd` — multi-root aware), `model`/`model_id`, `hook_event_name`, `status`.
- **`error` via `status` and a dedicated failure event**: confirmed (`cursor.com/docs/hooks`) `stop` and `subagentStop` both carry their own `status: "completed"|"aborted"|"error"` field — previously ignored, so an errored turn looked identical to a normal one. `postToolUseFailure` is a separate, dedicated failure-only event (its `postToolUse` counterpart fires on success), so — like Claude Code's `PostToolUseFailure` — mapping it to `error` can't become per-tool-call noise. `status: "aborted"` is left mapped to `task_complete`/`subagent_complete` for now (no confirmed dedicated handling), same treatment as an ordinary completion.
