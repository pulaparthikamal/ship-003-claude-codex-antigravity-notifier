...
**Fixed (2026-08-27)**: a report of "Antigravity fires for almost nothing, and Codex doesn't seem to work at all" led to auditing every adapter's hook registration and dispatch path against each agent's real, current docs (not just the payload shapes captured previously) — found and fixed several real root causes (a separate, unrelated fix from earlier the same day — per-agent enable/disable settings — is listed next, below):
1. **Codex's `hooks.json` schema was wrong — flat event names at the document root, with no `"hooks"` wrapper.** Confirmed against `learn.chatgpt.com/docs/hooks` that Codex requires the same top-level `"hooks"` wrapper Claude Code uses; the flat shape this adapter wrote was never recognized as a hook registration at all, so **Codex notifications never fired for anyone, on any platform**, regardless of the OS-specific sound/popup fixes already in place. Also wrong: hooking `PreToolUse` for `permission`, which fires on *every* tool call regardless of whether a human is involved — Codex's real approval-gate event is `PermissionRequest`, confirmed to fire only "when Codex is about to ask for approval." Fixed in `lib/adapters/codex.js`: correct wrapper schema, `PermissionRequest`→`permission`, added `SubagentStop`→`subagent_complete` for parity with Claude Code, and a Windows-specific `commandWindows` field (documented as a sibling of `command`, not something the old code ever set). `install()` migrates away from the old flat keys on re-install; `notifier doctor` flags a still-unmigrated file with a WARNING line. Found live evidence of the bug's real-world impact on a dev machine's own `~/.codex/hooks.json` before fixing it there too.
2. **Codex's `Stop`/`SubagentStop` hooks specifically require JSON on `stdout`** to be treated as a successful exit ("Plain text output is invalid for this event" — every other Codex event treats exit 0 + no output as success). `bin/notifier.js`'s `dispatch` command now prints `{"continue":true}` for exactly those two events when the agent is `codex`, and stays silent otherwise — Cursor's own hook contract explicitly forbids printing stdout it might try to parse as a decision, so this had to be conditional on the agent, not a blanket change.
3. **Antigravity's idle-heuristic timer never actually dispatched, ever** — `scheduleIdleCheck()` in `lib/adapters/antigravity.js` spawned `idle-check` with its `'antigravity'` argv element missing (present in Cursor's near-identical copy of the same function). Every later positional argument in `bin/notifier.js`'s `idle-check` case shifted left by one as a result: the marker file path landed in the wrong variable, `readMarker()` always got `undefined`, and the before/after timestamp comparison that gates the actual dispatch was always false. The detached timer process ran correctly on every `PostToolUse`; its one job — deciding whether to notify — was simply unreachable. One-line fix, verified end-to-end with a real dispatch + a shortened `NOTIFIER_ANTIGRAVITY_IDLE_MS`.
4. **Antigravity's `permission` notification is not fixable from this side today.** Re-confirmed against `antigravity.google/docs/hooks/`: `PreToolUse`'s real payload has no `requires_approval`-style field, and there's no separate "waiting for approval" event — gating happens entirely through the hook's own `decision` output, which is a control point, not an observation point. A previous version of [AGENT_INTEGRATIONS.md](AGENT_INTEGRATIONS.md#antigravity) implied this was implementable ("adapter must inspect the tool metadata itself") — that line was aspirational and never matched the real payload; corrected to state the gap plainly. If Antigravity ships a dedicated event for this, revisit; until then it's a genuine upstream API gap, not a bug here.
5. **Antigravity `task_complete` not firing at all is very likely the already-documented Windows upstream bug**, not something touched by fixes 1–4: a live report shows `Stop`/`PostToolUse` hooks never firing on Windows 11 (Antigravity IDE 1.107.0) even with the correct schema — see [AGENT_INTEGRATIONS.md](AGENT_INTEGRATIONS.md#antigravity). No code-level workaround exists for that from our side; if you're seeing missing `task_complete` notifications specifically on Windows, that's almost certainly it.

Cross-platform note: none of the fixes above are OS-specific themselves (the schema/stdout/argv bugs affect Codex and Antigravity identically on macOS/Windows/Linux) — the previously-fixed (2026-08-24) sound/popup backends per OS were already correct and untouched by this pass. The one remaining platform-specific gap is antigravity item 5 above (Windows-only, upstream, unfixed).

**Fixed (2026-08-27, earlier same day)**: a separate report — the extension writing `.cursor/hooks.json`/`.agents/hooks.json` into *every* workspace folder opened in VS Code/Antigravity, regardless of whether that project uses Cursor or Antigravity at all — traced to `install all` running unconditionally on every activation, never checking the `agents.<name>` config toggle that already existed in the schema for exactly this purpose (it was previously wired only into `engine.dispatch`'s post-hoc notification filtering, not into whether the hook file gets written in the first place). Fixed: `bin/notifier.js`'s `install` case now skips (and uninstalls) any agent set to `false` in config; the extension gained four matching settings — `notifier.enableClaude`/`enableCodex`/`enableAntigravity`/`enableCursor` (all default `true`, so default behavior is unchanged) — synced into that config before every `install all` run, so unchecking one both stops the file from being (re)created and removes any copy already written. Verified end-to-end in a scratch workspace: `agents.cursor: false` skips Cursor and strips the notifier's entries out of an existing `.cursor/hooks.json`, leaving other agents untouched.
...
- **Antigravity has no native "waiting on input" hook event today** — the idle-heuristic timer is a workaround, not equivalent fidelity to Claude Code's `idle_prompt`. ~~Was also silently non-functional due to a missing argv element~~ — fixed 2026-08-27, see the Implementation status entry above. Still a heuristic, not a real event: revisit if/when Antigravity ships a dedicated one (track the upstream issue referenced in AGENT_INTEGRATIONS.md).
- **Antigravity's `permission` kind is not implementable against the hook API as it exists today** — confirmed 2026-08-27 against `antigravity.google/docs/hooks/`: `PreToolUse`'s real payload has no approval-required flag, and gating happens only through the hook's own `decision` output (a control point, not an observable event). Not a bug to fix; revisit only if Antigravity ships a dedicated approval-observation event. See [AGENT_INTEGRATIONS.md](AGENT_INTEGRATIONS.md#antigravity).
- ~~Codex's legacy `--notify` vs. `hooks.json`~~ — resolved 2026-08-27: current Codex docs (`learn.chatgpt.com/docs/hooks`) make no mention of a legacy `--notify`/`notify` path; don't add one without a concrete report from a user on an old Codex version that needs it. The real, separate bug found in the same pass — `hooks.json`'s schema being wrong, so Codex notifications never fired for anyone — is fixed; see the Implementation status entry above.
